...
GPUI-independent and lives in `src/model/history/`. Selection history remains
in the view layer using `gpui_component::History`, because it has different
ownership and lifecycle requirements. This separation was driven by
...
while keeping history logic testable without a UI framework.

...
use gauss_core::model::{Document, HitTestIndex, SelectPointerHit, ShapeId, Vec2};
    ...
    fn matches_hit(self, hit: SelectPointerHit, expected_shape: ShapeId) -> bool {
        match self {
            Self::Handle { anchor_index } => matches!(
                hit,
                SelectPointerHit::Handle(h)
                    if h.shape_id == expected_shape && h.anchor_index == anchor_index
            ),
            Self::Segment { seg_index } => matches!(
                hit,
                SelectPointerHit::Segment(s)
                    if s.shape_id == expected_shape && s.seg_index == seg_index
            ),
            Self::TopmostShape { shape_index } => matches!(
                hit,
                SelectPointerHit::Shape(s)
                    if s.shape_id == expected_shape && s.shape_index == shape_index
            ),
        }
    }
}

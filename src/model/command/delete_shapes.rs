...
/// # Errors
///
/// Returns `UserError::InvalidOperation` if any target index is out of range.
...
///
/// Returns `UserError::InvalidOperation` if any target index is out of range.
pub fn apply_restore_shapes(doc: &mut Document, targets: &[DeletedShape]) -> Result<(), UserError> {

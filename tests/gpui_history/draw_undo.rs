...
fn assert_draw_shape_absent(
    ...
    if crate::common::find_draw_shape(doc).is_some() {
        return Err(TestSupportError::expectation(format!(
...
fn click_and_verify_state(
    ...
    crate::common::click_canvas_and_wait(visual_cx, verification.position);
...
fn stale_active_path_is_recovered_when_pen_draws_new_shape(cx: &mut TestAppContext) {
    ...
    crate::common::click_canvas_and_wait(visual_cx, click_point);

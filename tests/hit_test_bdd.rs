...
fn assert_pointer_hit_matches<F>(
    world: &HitTestWorld,
    spec: HitAssertionSpec,
    check: F,
) -> TestSupportResult<()>
where
    F: Fn(SelectPointerHit, ShapeId) -> bool,
{
    let expected_shape = (spec.shape_selector)(&world.shape_ids)
        .copied()
        .ok_or_else(|| TestSupportError::missing("shape_id", spec.assertion_label))?;
    let hit = require_pointer_hit(world)?;
    if !check(hit, expected_shape) {
        return Err(TestSupportError::expectation(format!(
            "{} for {expected_shape:?}; got {hit:?}",
            spec.expected_desc
        )));
    }
    Ok(())
...
fn then_result_handle(world: &HitTestWorld) -> TestSupportResult<()> {
    assert_pointer_hit_matches(
        world,
        HitAssertionSpec {
            shape_selector: <[ShapeId]>::first,
            assertion_label: "handle assertion",
            expected_desc: "expected Handle hit",
        },
        |hit, expected_shape| {
            matches!(hit, SelectPointerHit::Handle(h)
                if h.shape_id == expected_shape && h.anchor_index == 0)
        },
    )
}
...
fn then_result_segment(world: &HitTestWorld) -> TestSupportResult<()> {
    assert_pointer_hit_matches(
        ...
        HitAssertionSpec {
            shape_selector: <[ShapeId]>::first,
            assertion_label: "segment assertion",
            expected_desc: "expected Segment hit",
        },
        |hit, expected_shape| {
            matches!(hit, SelectPointerHit::Segment(s)
                if s.shape_id == expected_shape && s.seg_index == 0)
        },
    )
}
...
fn then_result_topmost_shape(world: &HitTestWorld) -> TestSupportResult<()> {
    assert_pointer_hit_matches(
        world,
        HitAssertionSpec {
            shape_selector: <[ShapeId]>::last,
            assertion_label: "topmost assertion",
            expected_desc: "expected topmost Shape hit",
        },
        |hit, expected_shape| {
            matches!(hit, SelectPointerHit::Shape(s)
                if s.shape_id == expected_shape && s.shape_index == 1)
        },
    )
}
